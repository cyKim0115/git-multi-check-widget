...
15. 사고·판단·구현은 고성능 채팅에서 하고, **확정된 문서 Stage**(템플릿 채우기·brief 확장)만 값싼 서브에이전트(예: Composer)에 넘길 수 있다. 판단 자체를 넘기지 않으며, 짧은 handoff brief가 있어야 토큰 절감이 된다 (`workflows/cheap-subagent-handoff.md`).
...
문서 Stage 비용 절감: `workflows/cheap-subagent-handoff.md`.
